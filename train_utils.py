...
# =============================================================================
# LLM Adversarial Designer (Optional - requires API key)
# =============================================================================

class LLMAdversarialDesigner:
    """
    LLM-powered adversarial case generator.
    
    Uses an LLM (Claude/GPT) to design trap cases that target
    the agent's specific weaknesses. This creates more sophisticated
    and unpredictable challenges than rule-based generation.
    
    Requires: ANTHROPIC_API_KEY or OPENAI_API_KEY environment variable.
    """
    
    SYSTEM_PROMPT = '''You are an adversarial designer for a loan underwriting training environment.

Your job is to create loan applications that will TRICK an AI agent into making wrong decisions.
The agent has learned basic rules but struggles with edge cases.

RULES THE AGENT MUST FOLLOW:
- CIBIL score < 700 → reject (even if everything else is perfect)
- FOIR > 50% → reject (even with co-applicant)
- Missing documents → request_docs (before any other decision)
- Home loan without RERA → reject (regulatory requirement)
- Vehicle LTV > 85% → counter_offer
- Home loan LTV tiers: ≤30L→90%, 30-75L→80%, >75L→75%
- Employment: Personal/Vehicle need 1+ years, Home needs 2+ years

AGENT'S CURRENT WEAKNESSES:
{weaknesses}
...
YOUR TASK:
Create a loan application JSON that will likely trick the agent.
Make it look VERY approvable at first glance, but have ONE hidden flaw.

The best traps:
1. High income but CIBIL just below 700 (699, 698)
2. Perfect financials but RERA=false
3. Excellent credit but FOIR at 51-52%
4. Great profile but LTV exactly at/above tier limit
5. Everything perfect but employment just below threshold

OUTPUT FORMAT (JSON only, no explanation):
{
    "loan_type": "personal" | "vehicle" | "home",
    "credit_score": <number 650-850>,
    "monthly_income": <number 50000-1000000>,
    "foir": <float 0.15-0.60>,
    "employment_years": <number 0-20>,
    "loan_amount": <number 100000-20000000>,
    "documents_complete": true | false,
    "collateral_value": <number if vehicle/home>,
    "ltv_ratio": <float 0.5-0.95 if vehicle/home>,
    "rera_registered": true | false (if home),
    "has_co_applicant": true | false (if home),
    "trap_type": "<brief description of the trap>",
    "expected_decision": "approve" | "reject" | "request_docs" | "counter_offer"
}'''

    def __init__(self, provider: str = "anthropic", model: str = None):
        """
        Initialize LLM adversarial designer.
        
        Args:
            provider: "anthropic" or "openai"
            model: Model name (default: claude-3-haiku or gpt-4o-mini)
        """
    ...
    def design_trap(self, weaknesses: dict, recent_failures: list = None) -> dict:
        """
        Design a trap case targeting agent's weaknesses.
        
        Args:
            weaknesses: Dict of strategy → failure_rate
        ...
        weakness_str = "\n".join([
            f"- {strategy}: {rate*100:.0f}% failure rate"
            for strategy, rate in sorted(weaknesses.items(), key=lambda x: -x[1])[:5]
        ]) or "No weakness data yet"
        
        failures_str = "\n".join(recent_failures[-5:]) if recent_failures else "No recent failures"
        ...
        try:
            if self.provider == "anthropic":
                ...
                response = self.client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": "Generate one adversarial loan application."}
                    ]
                )
            ...
            # Add vehicle/home specific fields if missing
            if case["loan_type"] in ("vehicle", "home") and "collateral_value" not in case:
                case["collateral_value"] = case["loan_amount"] * 1.3
            ...
            print(f"LLM adversarial failed: {e}, falling back to rule-based")
            return generate_adversarial_case()
    
    def generate_batch(self, num_cases: int, tracker: 'AdversarialTracker' = None) -> list:
        """Generate a batch of adversarial cases."""
        cases = []
        weaknesses = tracker.get_summary() if tracker else {}
        weakness_rates = {s: d.get("accuracy", 0.5) for s, d in weaknesses.items()}
        # Invert: high accuracy = low weakness
        weakness_rates = {s: 1 - r for s, r in weakness_rates.items()}
        
        for _ in range(num_cases):
            case = self.design_trap(weakness_rates)
            cases.append(case)
...
# =============================================================================
# Adversarial Case Generator (Rule-Based - Default)
# =============================================================================
